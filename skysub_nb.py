...
def skysub_nb(inlist, inpref='', outpref='s', mskpref='none', ndthpos=9, iternum=50, nsigma=3, second=False, step=0.05, fringe=False, fr_x0=0.0, fr_y0=0.0, fr_step=5):
    ...
    for i in range(n_subset):
        ...
        for j in range(len(in_array[i])):
            ...
            if fringe:
                if mask == 1:
                    #print mask_array[i][j]
                    mask_im = pyfits.open(mask_array[i][j])
                    if not second:
                ...
                while r1 <= rmax:
                    ...
                    if mask == 1:
                        ...
                        med = np.nanmedian(out_subset[idx_sky])
                    else:
                        med = np.nanmedian(out_data[idx])
...
if __name__=="__main__":
    ...
    skysub_nb(args[0], inpref=options.inpref, outpref=options.outpref, mskpref=options.mskpref, ndthpos=options.ndthpos, iternum=options.iternum, nsigma=options.nsigma, second=options.second, step=options.step, fringe=options.fringe, fr_x0=options.fr_x0, fr_y0=options.fr_y0, fr_step=options.fr_step)
